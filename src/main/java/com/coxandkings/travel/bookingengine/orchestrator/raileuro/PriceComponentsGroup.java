...
import com.coxandkings.travel.bookingengine.orchestrator.raileuro.PriceComponent;
...
//	
	...
	public void add(String compCode, String compCcy, BigDecimal compAmt) {
		...
		if (currCompCode.endsWith(Constants.SIGMA)) {
			isSummation = true;
			currCompCode = currCompCode.substring(0, currCompCode.length() - Constants.SIGMA.length());
		}
	...
	public Object toJSON() {
		...
		if (mDoSummation) {
			...
			prcCompGrpJson.put("amount", getComponentAmount());
			prcCompGrpJson.put("currencyCode", getComponentCurrency());
