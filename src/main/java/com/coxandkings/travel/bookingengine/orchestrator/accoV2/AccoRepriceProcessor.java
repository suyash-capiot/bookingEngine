...
public class AccoRepriceProcessor implements AccoConstants {
	...
	public static String opsProcess(JSONObject reqJson) throws RequestProcessingException, InternalProcessingException  {
		...
		try {
			...
			Map<Integer,String> SI2BRMSRoomMap= new HashMap<Integer,String>();
			JSONObject resSupplierComJson =  SupplierCommercials.getSupplierCommercials(CommercialsOperation.Book,reqJson, resJson, SI2BRMSRoomMap);;
	...
	private static void pushSuppFaresToRedisAndRemove(JSONObject reqJson,JSONObject resJson) throws Exception {
		...
		for(int j=0; j < multiResJsonArr.length(); j++) {
			...
			redisReqKey =getRedisKeyForReq((JSONObject) multiReqJsonArr.get(j));//Make key using Req ka accomodation Info 
			for (int i=0; i < roomStayJsonArr.length(); i++) {
				JSONObject roomStayJson = roomStayJsonArr.getJSONObject(i);//Gets Rooms
				...
				redisRoomKey = redisReqKey.concat(getRedisKeyForRoomStay(roomStayJson.getJSONObject(JSON_PROP_ROOMINFO)));//Make key of reqkey+roomstayjson
	...
	static String getRedisKeyForReq( JSONObject subReqJson) {
		...
		//Room index added here as it is added in multi room req
		return String.format("%s%c%s%c%s%c%s%c%s%c%d",subReqJson.getString(JSON_PROP_SUPPREF),KEYSEPARATOR,
				subReqJson.getString(JSON_PROP_COUNTRYCODE),KEYSEPARATOR,subReqJson.getString(JSON_PROP_CITYCODE),KEYSEPARATOR,
				subReqJson.getString(JSON_PROP_CHKIN),KEYSEPARATOR,subReqJson.getString(JSON_PROP_CHKOUT),KEYSEPARATOR,subReqJson.opt(JSON_PROP_ROOMINDEX));
	}

	static String getRedisKeyForRoomStay(JSONObject roomInfoJson) {

		//Room index removed here as it is added in every request during multi room handling
		return String.format("%c%s%c%s%c%s%c%s%c%s",KEYSEPARATOR,roomInfoJson.getJSONObject(JSON_PROP_HOTELINFO).getString(JSON_PROP_HOTELCODE),KEYSEPARATOR,
				...
				KEYSEPARATOR,roomInfoJson.getJSONObject(JSON_PROP_RATEPLANINFO).getString(JSON_PROP_RATEPLANCODE),KEYSEPARATOR,roomInfoJson.getJSONObject(JSON_PROP_ROOMTYPEINFO).getString(JSON_PROP_ROOMREF));
