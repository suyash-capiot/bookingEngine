...
public class TaxEngine implements CarConstants {
	...
	public static void getCompanyTaxes(JSONObject reqJson, JSONObject resJson) {
       ...
       /* gciJson.put(JSON_PROP_COMPANYNAME, "CnK");
        ...
        createSupplierAndTravelDetails(subType, usrCtx, carRentalJsonArr, suppHdrJsonMap, suppTaxReqJson);
	...
	private static void createSupplierAndTravelDetails(SubType subType, UserContext usrCtx, JSONArray carRentalInfoArr, Map<String,JSONObject> suppHdrJsonMap,Map<String, Map<String, JSONObject>> suppTaxReqJson) {
		OrgHierarchy orgHierarchy = usrCtx.getOrganizationHierarchy();
        for (int i=0; i < carRentalInfoArr.length(); i++) {
        	
        	//TODO: Currently getting multiple cars from SI response in reprice for a single request.
        	...
        	for(int j=0; j < vehicleAvailArr.length() ;j++) {
	        	...
	        	JSONObject totalsplEquipJson = totalFareJson.optJSONObject(JSON_PROP_SPLEQUIPS);
	        	JSONArray splEquipJsonArr = (totalsplEquipJson != null) ? totalsplEquipJson.getJSONArray(JSON_PROP_SPLEQUIP) : null;
	        	//TODO : City might have to be deduced from dropOff Location.
	        	...
	        	if (splEquipJsonArr != null) {
	        		for (int k=0; k < splEquipJsonArr.length(); k++) {
	            		...
	            		JSONObject fareBrkpJson = new JSONObject();
	            		fareBrkpJson.put(JSON_PROP_NAME, splEquipJson.getString(JSON_PROP_EQUIPTYPE));
	            		fareBrkpJson.put(JSON_PROP_VALUE, splEquipJson.getBigDecimal(JSON_PROP_AMOUNT));
	            		fareBrkpJsonArr.put(fareBrkpJson);
	        		}
	        	}
	        	...
	        	fareDtlsJson.put(JSON_PROP_SUBTYPE, PROD_CATEG_SUBTYPE_CAR);
...
//				String travelDtlsMapKey = String.format("%s|%s|%s", "Mumbai", "Maharashtra", "India");
	...
	private static boolean isMarkedUpPrice(JSONObject carTotalPriceJson) {
		JSONArray clEntityTotalCommsJsonArr = carTotalPriceJson.optJSONArray(JSON_PROP_CLIENTENTITYTOTALCOMMS);
