...
import com.coxandkings.travel.bookingengine.orchestrator.accoV2.AccoSearchProcessor;
	...
	public void process(ProductSupplier prodSupplier, JSONObject reqJson, Element reqElem) {
		...
		try {
		 ...
		 Document ownerDoc = reqElem.getOwnerDocument();
			...
			transactionElem.setTextContent(String.format("%s-%s-%s-%d", transactionElem.getTextContent(),prodSupplier.getSupplierID(),
					reqJson.getJSONObject(JSON_PROP_REQBODY).getString(JSON_PROP_ACCOSUBTYPE),reqJson.getJSONObject(JSON_PROP_REQBODY).optInt(JSON_PROP_ROOMINDEX)));
		}
		catch(Exception e){
		...
		try {
			...
			//TODO:This should not happen if there is an error calling supplier or client Commercials
			mSearchListener.receiveSupplierResponse(resJson);
		}
	}
