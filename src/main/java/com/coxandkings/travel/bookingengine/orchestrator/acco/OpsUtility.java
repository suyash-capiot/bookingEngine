...
public class OpsUtility implements AccoConstants{
	...
	public static String getKafkaRQ(JSONObject opsRqJson) throws Exception {
		
		JSONObject ackResponse = new JSONObject();
		JSONObject rqHdr = new JSONObject();
		
		try {
			
			rqHdr = opsRqJson.getJSONObject("requestHeader");
			UserContext contextV2 = UserContext.getUserContextForSession(rqHdr);
					
			AccoBookProcessor.kafkaRequestJson(opsRqJson, contextV2);
		}catch(Exception e) {
			e.printStackTrace();
			ackResponse.put("status", "500");
			ackResponse.put("id", "false");
			return ackResponse.toString();
		}
		ackResponse.put("status", "200");
		ackResponse.put("id", "true");
		return ackResponse.toString();
	}
