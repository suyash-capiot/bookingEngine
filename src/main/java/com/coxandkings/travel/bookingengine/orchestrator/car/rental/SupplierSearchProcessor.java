...
public class SupplierSearchProcessor extends TrackableExecutor implements CarConstants {
	...
	private RentalSearchListenerResponder mSearchListener;
	private JSONArray mProdSuppsJsonArr;
	
	SupplierSearchProcessor(RentalSearchListenerResponder searchListener, ProductSupplier prodSupplier, JSONObject reqJson, Element reqElem) {
	...
	public void process(ProductSupplier prodSupplier, JSONObject reqJson, Element reqElem) {
      ...
      //-----------------------------------------------------------------------------------
        ...
        if(CarConfig.ismEnableCacheRental()) {
			try { 
			    ...
			    if (commCacheRes != null && (commCacheRes.equals("error")) == false) {
			    	...
			    	//Put Supplier fares and Commercials in Redis and Remove
			    	RentalSearchProcessor.pushSuppFaresToRedisAndRemove(cachedResJson);
        ...
        try {
			...
			//Because each thread will hit a single supplier. Ideally there will be a single wrapper.
			...
			for (Element wrapperElem : wrapperElems) {
				...
				RentalSearchProcessor.getSupplierResponseVehicleAvailJSON(resBodyElem, vehicleAvailJsonArr);
			}
			...
			for (int i = 0; i < vehicleAvailJsonArr.length(); i++) {
				...
				vehicleAvail.put(JSON_PROP_BOOKINGDURATION, carRentalReq.optString(JSON_PROP_BOOKINGDURATION));
           ...
           JSONObject resClientJson = ClientCommercials.getClientCommercialsV2(reqJson, resJson, resSupplierJson);
			...
			RentalSearchProcessor.calculatePrices(reqJson, resJson, resSupplierJson, resClientJson, suppResToBRIIndex, true, usrCtx);
	        ...
	        RentalSearchProcessor.pushSuppFaresToRedisAndRemove(resJson);
