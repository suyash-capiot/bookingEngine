...
public class SupplierCommercials implements AccoConstants{
	...
	public static JSONObject getBRMSRoomDetailsJSON(JSONObject roomStayJson) {
		...
		BigDecimal totalPrice = totalPriceJson.getBigDecimal(JSON_PROP_AMOUNT);//totalpriceinfo amount
		roomDetailsJson.put("totalFare", String.valueOf(totalPrice));

		JSONObject taxesJson = totalPriceJson.optJSONObject(JSON_PROP_TOTALTAX);//taxes array in totalpriceinfo
