...
public class SupplierCommercials implements AccoConstants{
		...
		for(int i=0;i<multiResArr.length();i++) {
			...
			for(Object roomStayJson: accoInfoJson.getJSONArray(JSON_PROP_ROOMSTAYARR)) {
				...
				if(!prevSuppId.equals(suppId)) {
					...
					ProductSupplier prodSupp=usrCtx.getSupplierForProduct(PROD_CATEG_ACCO, ((JSONObject) roomStayJson).getString(JSON_PROP_ACCOSUBTYPE), suppId);
					
					commonElemsJson.put(JSON_PROP_SUPPMARKET,prodSupp.getSupplierMrkt());
					...
					briJsonArr.put(briIndex++,briJson);//bri index is for individual supp
				}
				...
				if (hotelDetailsJson == null) {
					...
					//////////////changes as told by tejal//////////////////
					cityName = RedisHotelData.getHotelInfo(hotelCode, JSON_PROP_CITY);
					cityAttrs = RedisCityData.getCityInfo(cityName);
					hotelDetailsJson.put(JSON_PROP_CONTINENT, cityAttrs.getOrDefault(JSON_PROP_CONTINENT,""));
					hotelDetailsJson.put(JSON_PROP_COUNTRY, cityAttrs.getOrDefault(JSON_PROP_COUNTRY,""));
					hotelDetailsJson.put(JSON_PROP_CITY, cityName);
					hotelDetailsJson.put(JSON_PROP_STATE, cityAttrs.getOrDefault(JSON_PROP_STATE,""));
	...
	public static JSONObject getBRMSRoomDetailsJSON(JSONObject roomStayJson) {
		...
		if(taxesJson!=null && taxesJson.has(JSON_PROP_AMOUNT)) {
			...
			fareBreakupJson.put("baseFare", totalPrice.subtract(taxesJson.getBigDecimal(JSON_PROP_AMOUNT)));//totalpriceinfo amt -taxes amt
