...
import com.coxandkings.travel.bookingengine.orchestrator.accoV2.AccoRepriceProcessor;
import com.coxandkings.travel.bookingengine.orchestrator.accoV2.enums.AccoSubType;
import com.coxandkings.travel.bookingengine.orchestrator.accoV2.enums.AvailabilityStatus;
...
public class AccoSearchProcessor implements AccoConstants {
	...
	public static JSONObject getSupplierResponseJSON(JSONObject reqJson,Element resElem){
        ...
        //UserContextV2 usrCtx=UserContextV2.getUserContextForSession(reqJson.getJSONObject(JSON_PROP_REQHEADER));
		...
		for(Element wrapperElem:XMLUtils.getElementsAtXPath(resElem, "./accoi:ResponseBody/acco:OTA_HotelAvailRSWrapper")) {
			for (Element roomStayElem : XMLUtils.getElementsAtXPath(wrapperElem, "./ota:OTA_HotelAvailRS/ota:RoomStays/ota:RoomStay")) {
				...
				//This is done assuming that roomIndex will b polpulated during multi room async search
				roomStayJson.put(JSON_PROP_ROOMINDEX, reqJson.getJSONObject(JSON_PROP_REQBODY).optInt(JSON_PROP_ROOMINDEX));
				
				//////////////////////////////////////////////////////////////////////////
			/*	AccoSubType prodCategSubtype = AccoSubType.forString(roomStayJson.optString(JSON_PROP_ACCOSUBTYPE));
				ProductSupplier prodSupplier = usrCtx.getSupplierForProduct(PROD_CATEG_ACCO,prodCategSubtype!=null?prodCategSubtype.toString():"", roomStayJson.optString(JSON_PROP_SUPPREF));
	            List<String> products = usrCtx.getProductForSupplier(roomStayJson.optString(JSON_PROP_SUPPREF), prodSupplier.getCredentialsName());
				if(products!=null && products.size()!=0) {
					if(!roomStayJson.getJSONObject(JSON_PROP_ROOMINFO).getJSONObject(JSON_PROP_HOTELINFO).getString(JSON_PROP_HOTELNAME).isEmpty()) {
						if(!products.contains(roomStayJson.getJSONObject(JSON_PROP_ROOMINFO).getJSONObject(JSON_PROP_HOTELINFO).getString(JSON_PROP_HOTELNAME))){
							continue;	
							}		
					}
					else {
						String hotelName = RedisHotelData.getHotelInfo(roomStayJson.getJSONObject(JSON_PROP_ROOMINFO).getJSONObject(JSON_PROP_HOTELINFO).getString(JSON_PROP_HOTELCODE), "name");
						if(!products.contains(hotelName)){
							continue;	
							}	
					}
	...
	public static JSONObject getRoomStayJSON(Element roomStayElem) {
		...
		//System.out.println(XMLTransformer.toString(TPAExtHotlFee));
		...
		//System.out.println(XMLTransformer.toString(TPAExtRoomRate));
	...
	private static JSONArray getSupplierOfferJson(Element tPAExtHotlFee) {
		...
		for(Element offerElem:XMLUtils.getElementsAtXPath(tPAExtHotlFee, "./acco:offers/acco:offer")) {
			...
			//TODO:Need to added OfferType:Online/Offline
	...
	public static JSONObject getRoomInfoJson(Element roomStayElem) {
		...
		//As multiroom implementation is done,just map rph so that it can be passed to other calls
		...
		roomInfoJson.put(JSON_PROP_ROOMRPH, XMLUtils.getValueAtXPath(roomStayElem, "./@RPH"));
	...
	public static void calculatePrices(JSONObject reqJson, JSONObject resJson, JSONObject clientCommResJson,JSONObject suppCommResJson, Map<Integer,String> SI2BRMSRoomMap,boolean retainSuppFaresAndComm,Date cutOffDate) {
		...
		for(int i=0;i<resAccoInfoJsonArr.length();i++) {
			...
			for(int j=0;j<resRoomJsonArr.length();j++) {
				...
				//newTotalPriceJson.put(JSON_PROP_INCENTIVES, incentivesArray);
				...
				if(ccommRoomJson==null) {
					logger.trace(String.format("Client commercials room detail not found at briKey %s for room number %s",SI2BRMSRoomMap.get(roomIdx-1),roomIdx-1));
					continue;
				}
	...
	private static JSONArray getClntEntityCommJsonArray(JSONArray entityCommJsonArr,Map<String,String> ccommTypeMap,JSONObject reqHdr,String supplierCurrency) {
		...
		for(int i=0;i<entityCommJsonArr.length();i++) {
			JSONObject clntentityJson = entityCommJsonArr.getJSONObject(i);
			addClientEntityJson(clntEntityCommJsonArr,clntentityJson, ccommTypeMap,reqHdr,supplierCurrency,i);	
		}
		//entityCommJsonArr.forEach(clntentityJson -> addClientEntityJson(clntEntityCommJsonArr,(JSONObject) clntentityJson, ccommTypeMap,reqHdr,supplierCurrency,i++));

		return clntEntityCommJsonArr;
	}

	private static void addClientEntityJson(JSONArray clntEntityJsonArr, JSONObject clntentityJson,
			Map<String,String> ccommTypeMap,JSONObject reqHdr,String supplierCurrency,int i) {
		...
		for(int y=0;y<clientEntityDetailsArr.length();y++) {*/
			...
			entityJson.put(JSON_PROP_CLIENTID, clientEntityDetailsArr[i].getClientId());
			entityJson.put(JSON_PROP_PARENTCLIENTID, clientEntityDetailsArr[i].getParentClienttId());
			entityJson.put(JSON_PROP_COMMENTITYTYPE, clientEntityDetailsArr[i].getCommercialsEntityType());
			entityJson.put(JSON_PROP_COMMENTITYID, clientEntityDetailsArr[i].getCommercialsEntityId());
