...
public class CarBookProcessor implements CarConstants{
	...
	public static String process(JSONObject reqJson) throws InternalProcessingException, RequestProcessingException{
		...
		try {
			...
			validateRequestParamter(reqJson);
	...
	private static void validateRequestParamter(JSONObject reqJson) throws ValidationException {
		
		CarRequestValidator.validatePaxDetails(reqJson);
		CarRequestValidator.validatePaymentInfo(reqJson);
		CarRequestValidator.validatebookID(reqJson);
	}
...
//       rentalPaymentPrefElem.appendChild(paymentCardElem);
	...
	private static void sendPreBookingKafkaMessage(KafkaBookProducer bookProducer, JSONObject reqJson,  UserContext usrCtx) throws Exception {
    	...
    	kafkaMsgJson.getJSONObject(JSON_PROP_REQBODY).put(JSON_PROP_PRODCATEGSUBTYPE, PROD_CATEG_SUBTYPE_CAR);
