...
public class CarSearchSorterResponder implements CarConstants {
	
	private static final DecimalFormat decFmt = new DecimalFormat("000000000.000000"); 
	private static final Logger logger = LogManager.getLogger(CarSearchSorterResponder.class);
	...
	private static JSONArray sortVehicleAvailByPrice(List<JSONArray> itinsList) {
		...
		for (JSONArray itins : itinsList) {
			for (int i=0; i < itins.length(); i++) {
				...
				strBldr.append(CarPriceProcessor.getRedisKeyForVehicleAvail(vehicleAvail));
